...
export default function StudentMessages() {
  ...
  // ---------- Helper to safely get conversation fields ----------
  const getOtherUserName = (chat) =>
    chat.other_user_name || chat.name || "Unknown";
  const getOtherUserRole = (chat) =>
    chat.other_user_role || chat.role || "user";
  const getLastMessageTime = (chat) =>
    chat.last_message_time || chat.updated_at || chat.created_at;
  const getUnreadCount = (chat) => chat.unread_count ?? chat.unread ?? 0;
  const getCourseName = (chat) => chat.course_name || chat.course;
  ...
  const handleSearchUsers = async () => {
    ...
    try {
      const res = await api.get(
        `/messages/search?q=${searchUsers}&role=${filterRole}`,
      );
  ...
  useEffect(() => {
    ...
    newSocket.on("user_online", ({ userId, userRole }) => {
      setConversations((prev) =>
        prev.map((c) => {
          const otherId = c.other_user_id || c.participant2_id;
          const otherRole = c.other_user_role || c.participant2_role;
          if (otherId === userId && otherRole === userRole) {
  ...
  const handleSendMessage = async () => {
    ...
    socket.emit("send_message", {
      receiverId: selectedChat.other_user_id || selectedChat.participant2_id,
      receiverRole:
        selectedChat.other_user_role || selectedChat.participant2_role,
      message: messageText,
  ...
  const filteredConversations = conversations.filter(
    (c) =>
      ...
      getCourseName(c)?.toLowerCase().includes(search.toLowerCase()),
  );
  ...
  const instructorCount = conversations.filter(
    (c) => getOtherUserRole(c) === "instructor",
  ).length;
  const studentCount = conversations.filter(
    (c) => getOtherUserRole(c) === "student",
  ).length;
    ...
    <div className="min-h-screen bg-gray-100 p-1 pr-0">
            ...
            <div className="flex-1 overflow-y-auto">
              {loading ? (
                ...
                filteredConversations.map((chat) => {
                  ...
                  const otherRole = getOtherUserRole(chat);
                  const lastTime = getLastMessageTime(chat);
                  const unreadCount = getUnreadCount(chat);
                  const courseName = getCourseName(chat);
                  ...
                  return (
                    <div
                      ...
                      className={`p-3 border-b border-gray-100 cursor-pointer hover:bg-gray-50 transition ${selectedChat?.id === chat.id ? "bg-blue-50" : ""}`}
                    >
                      <div className="flex items-center gap-3">
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <div className="flex items-center gap-2 mt-1">
                            <span
                              className={`inline-flex items-center gap-1 text-xs px-2 py-0.5 rounded-full capitalize ${getRoleBadgeColor(otherRole)}`}
                            >
                              {getRoleIcon(otherRole)} {otherRole}
                            </span>
                            {courseName && (
                  ...
                  <div>
                    ...
                    <p className="text-xs text-gray-500">
                      ...
                      <span className="capitalize">
                        {getOtherUserRole(selectedChat)}
                      </span>
                      {getCourseName(selectedChat) &&
                        ` • ${getCourseName(selectedChat)}`}
                    </p>
                  </div>
                </div>
                    ...
                    <div className="max-w-[70%]">
                      {msg.sender_id !== currentUser.id && (
                        <div className="flex items-center gap-1 mb-1 ml-2">
                          <span
                            className={`text-xs px-1.5 py-0.5 rounded-full capitalize ${getRoleBadgeColor(msg.sender_role)}`}
                          >
                      ...
                      <div
                        className={`rounded-2xl px-4 py-2 ${msg.sender_id === currentUser.id ? "bg-blue-600 text-white" : "bg-white text-gray-800 shadow-sm border border-gray-100"}`}
                      >
                        <p className="text-sm break-words">{msg.message}</p>
                      </div>
                      <div
                        className={`flex items-center gap-1 mt-1 text-xs text-gray-400 ${msg.sender_id === currentUser.id ? "justify-end" : "justify-start"}`}
                      >
                        <span>
              ...
              <div className="flex gap-2 mb-4">
                {["all", "instructor", "student"].map((role) => (
                  <button
                    ...
                    className={`flex-1 px-3 py-1.5 text-sm rounded-lg capitalize ${filterRole === role ? "bg-blue-600 text-white" : "bg-gray-100 text-gray-600"}`}
                  >
                    {role}
                  </button>
                ))}
      ...
      {toast.show && (
        <div className="fixed bottom-6 right-6 z-50 animate-slideIn">
          <div
            className={`bg-gray-900/95 text-white px-5 py-3 rounded-xl shadow-2xl border-l-4 flex items-center gap-3 ${toast.type === "success" ? "border-green-500" : "border-red-500"}`}
          >
            <div
              className={
