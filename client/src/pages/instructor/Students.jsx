...
export default function Students() {
  ...
  const fetchStudents = async () => {
    try {
      ...
      // Ensure each student has a courseId (fallback to course.id if needed)
      const normalizedStudents = (res.data || []).map((s) => ({
        ...s,
        courseId: s.courseId ?? s.course?.id ?? null,
        courseTitle: s.courseTitle ?? s.course?.title ?? "Unknown Course",
      }));
  ...
  // Fetch instructor's own courses for filter dropdown
  ...
  // **FIXED FILTER**: convert courseId to string for comparison with selectedCourse (string)
  const filteredStudents = students.filter((student) => {
    ...
    const matchesCourse =
      selectedCourse === "all" ||
      String(student.courseId) === String(selectedCourse);
  ...
  const totalEnrollments = students.length; // each row is one enrollment
  const avgProgress =
    students.length > 0
      ? Math.round(
                ...
                <div className="flex flex-col lg:flex-row lg:justify-between lg:items-center gap-4">
                  <div>
                    ...
                    <p className="text-xs text-gray-400 mt-0.5">
                      Browse and search all enrolled students
                    </p>
                  </div>
                      ...
                      {filteredStudents.map((student, idx) => (
                        <tr
                          ...
                          className={`border-b border-gray-50 hover:bg-gray-50/50 transition ${
                            idx !== filteredStudents.length - 1 ? "border-b" : ""
                          }`}
                        >
                          ...
                          <td className="py-3 px-5">
                            <div className="flex items-center justify-center gap-2">
                              ...
                              <button
                                onClick={() =>
                                  navigate(`/instructor/message/${student.id}`)
                                }
